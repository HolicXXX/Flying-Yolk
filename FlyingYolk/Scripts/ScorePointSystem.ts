      ...
      this.world.forEach([ut.Entity, game.ScorePoint, ut.Core2D.TransformObjectToWorld],
        (entity, scorepoint, o2w) => {
          ...
          if (position.x < playerPosition.x) {
            ...
            const cspeed = gameConfig.scrollSpeed + 0.001 * Math.floor(gameConfig.currentScore / 10);
            if(cspeed > gameConfig.currentScrollSpeed) {
              game.AudioService.playAudioSourceByName(this.world, 'audio/sfx_swooshing');
              gameConfig.currentScrollSpeed = cspeed;
            }else {
